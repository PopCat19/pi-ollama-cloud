...
export default async function (pi: ExtensionAPI) {
  ...
  try {
    ...
    if (!resp.ok) {
      console.error(
        `[ollama-cloud] /api/tags returned ${resp.status} ${resp.statusText} – falling back to models.json`
      );
      return;
    }
...
function buildModelConfig(entry: OllamaModelEntry): ProviderModelConfig {
  ...
  const hasThinking =
    explicitCaps.includes("thinking") || matchesAny(id, THINKING_PATTERNS);
  const hasVision =
    explicitCaps.includes("vision") || matchesAny(id, VISION_PATTERNS);
